 ...
 * The synced time differs from Date.now() in that it uses a time synced with the app server,
...
export function mongoWhere<T>(o: any, selector: MongoQuery<T>): boolean {
	...
	_.each(selector, (s: any, key: string) => {
		...
		try {
			...
			console.log(e || (e as any).reason || (e as any).toString()) // todo: why this logs empty message for TypeError (or any Error)?
			ok = false
		}
	})
	return ok
